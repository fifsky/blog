...
class RemindService {
    ...
    func list(page: Int) async throws -> RemindListResponse {
        ...
        return try await APIClient.shared.request(path: "/blog/admin/remind/list", body: request, auth: true)
    }

    // 创建提醒
    func create(cron: String?, content: String) async throws -> IDResponse {
        let request = RemindCreateRequest(cron: cron, content: content)
        return try await APIClient.shared.request(path: "/blog/admin/remind/create", body: request, auth: true)
    }
    ...
    func update(id: Int, cron: String?, content: String?, status: String?) async throws -> IDResponse {
        ...
        return try await APIClient.shared.request(path: "/blog/admin/remind/update", body: request, auth: true)
    }
    ...
    func delete(id: Int) async throws {
        ...
        let _: EmptyResponse = try await APIClient.shared.request(path: "/blog/admin/remind/delete", body: request, auth: true)
    }
}
