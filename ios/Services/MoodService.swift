...
class MoodService {
    ...
    func list(page: Int) async throws -> MoodListResponse {
        ...
        return try await APIClient.shared.request(path: Config.moodListPath, body: request)
    }

    // 创建心情
    func create(content: String) async throws -> IDResponse {
        let request = MoodCreateRequest(content: content)
        return try await APIClient.shared.request(path: Config.adminMoodCreatePath, body: request, auth: true)
    }

    // 更新心情
    func update(id: Int, content: String?) async throws -> IDResponse {
        let request = MoodUpdateRequest(id: id, content: content)
        return try await APIClient.shared.request(path: Config.adminMoodUpdatePath, body: request, auth: true)
    }
    ...
    func delete(ids: [Int]) async throws {
        ...
        let _: EmptyResponse = try await APIClient.shared.request(path: Config.adminMoodDeletePath, body: request, auth: true)
    }
}
