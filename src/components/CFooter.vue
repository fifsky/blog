<template>
    <div id="footer">
        <p>
            <a href="https://fangyuan.love" target=_blank>最好的我们</a> |
            <a href="https://caishuyan.com/" target=_blank>最好的她们</a> &copy;2022 <a href="https://github.com/fifsky/blog" target="_blank">fifsky.com</a>
        </p>

        <p class="info"><a href="https://beian.miit.gov.cn" target="_blank">沪ICP备14029559号-1</a> <a target="_blank" href="http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=31011702007077"><img src="data:image/png;base64,"/>沪公网安备 31011702007077</a>
        </p>
        <a id="scroll_top" href="javascript:void(0)" @click="top" v-show="showScroll">返回顶部</a>
    </div>
</template>

<script>
  export default {
    name: "CFooter",
    data() {
      return {
        showScroll: false
      }
    },
    methods:{
      top(){
        window.scroll(0,0)
      }
    },
    mounted() {
      window.onscroll = () => {
        this.showScroll = document.documentElement.scrollTop > 300
      }
    }
  }
</script>
