...
func TestRealWorldScenario(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if err := s.Register(&scheduler.Job{
			...
			t.Fatalf("failed to register quick-check job: %v", err)
		}
		...
		if err := s.Register(&scheduler.Job{
			...
			t.Fatalf("failed to register slow-process job: %v", err)
		}
		...
		// 在假时钟环境中等待 5 秒，synctest 会自动推进时间穿过所有调度事件
		...
		// 推进假时钟让 Timeout 中间件泄漏的 handler goroutine 完成
		// （context 取消后 Timeout 提前返回，但内部 goroutine 仍在 time.Sleep 中）
...
func TestCancellationDuringExecution(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if err := s.Register(&scheduler.Job{
			...
			t.Fatalf("failed to register long-job: %v", err)
		}
		...
		// 推进假时钟到任务第一次触发，等待 handler 开始执行并进入 select 阻塞
		time.Sleep(1 * time.Second)
		synctest.Wait()

		if !started.Load() {
			t.Error("job did not start within timeout")
		}
...
func TestErrorPropagation(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if err := s.Register(&scheduler.Job{
			...
			t.Fatalf("failed to register failing-job: %v", err)
		}
...
func TestPanicRecovery(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if err := s.Register(&scheduler.Job{
			...
			t.Fatalf("failed to register panicking-job: %v", err)
		}
...
func TestMultipleJobsWithDifferentSchedules(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		if err := s.Register(&scheduler.Job{
			...
			t.Fatalf("failed to register job-1sec: %v", err)
		}
		...
		if err := s.Register(&scheduler.Job{
			...
			t.Fatalf("failed to register job-2sec: %v", err)
		}
		...
		if err := s.Register(&scheduler.Job{
			...
			t.Fatalf("failed to register job-3sec: %v", err)
		}
		...
		// 在假时钟环境中等待 6 秒，synctest 会自动推进时间穿过所有调度事件
